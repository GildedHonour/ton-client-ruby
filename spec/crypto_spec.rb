...
describe TonSdk::Crypto do
  context "methods of crypto" do
    ...
    it "#modular_power" do
      ...
      @res = TonSdk::Crypto.modular_power(@c_ctx.context, pr1)
    ...
    it "#ton_crc16" do
      ...
      @res = TonSdk::Crypto.ton_crc16(@c_ctx.context, pr1)
    ...
    it "#generate_random_bytes" do
      ...
      @res = TonSdk::Crypto.generate_random_bytes(@c_ctx.context, pr1)
    ...
    it "#sha256, sha512" do
      ...
      @res1 = TonSdk::Crypto.sha256(@c_ctx.context, pr1)
      ...
      @res2 = TonSdk::Crypto.sha512(@c_ctx.context, pr2)
    ...
    it "#convert_public_key_to_ton_safe_format" do
      ...
      @res1 = TonSdk::Crypto.convert_public_key_to_ton_safe_format(@c_ctx.context, pr1)
      expect(@res1.success?).to eq true
    ...
    it "#sign" do
      ...
      @res1 = TonSdk::Crypto.sign(@c_ctx.context, pr1)
      expect(@res1.success?).to eq true
    ...
    it "#verify_signature" do
      ...
      @res1 = TonSdk::Crypto.verify_signature(@c_ctx.context, pr1)
      expect(@res1.success?).to eq true
    ...
    it "#scrypt" do
      ...
      @res1 = TonSdk::Crypto.scrypt(@c_ctx.context, pr1)
      expect(@res1.success?).to eq true
    ...
    it "hdkey methods" do
      ...
      @res1 = TonSdk::Crypto.hdkey_xprv_from_mnemonic(@c_ctx.context, pr1)
      expect(@res1.success?).to eq true
      ...
      @res2 = TonSdk::Crypto.hdkey_secret_from_xprv(@c_ctx.context, pr2)
      expect(@res2.success?).to eq true
      ...
      @res3 = TonSdk::Crypto.hdkey_public_from_xprv(@c_ctx.context, pr3)
      ...
      @res4 = TonSdk::Crypto.hdkey_derive_from_xprv(@c_ctx.context, pr4)
      ...
      @res5 = TonSdk::Crypto.hdkey_derive_from_xprv_path(@c_ctx.context, pr5)
      expect(@res5.success?).to eq true
      expect(@res5.result.xprv).to eq "xprvA1KNMo63UcGjmDF1bX39Cw2BXGUwrwMjeD5qvQ3tA3qS3mZQkGtpf4DHq8FDLKAvAjXsYGLHDP2dVzLu9ycta8PXLuSYib2T3vzLf3brVgZ"
    end

    it "mnemonics methods" do
      # 1
      pr1 = TonSdk::Crypto::ParamsOfMnemonicWords.new()
      @res1 = TonSdk::Crypto.mnemonic_words(@c_ctx.context, pr1)
      expect(@res1.success?).to eq true
      ...
      @res2 = TonSdk::Crypto.mnemonic_from_entropy(@c_ctx.context, pr2)
      expect(@res2.success?).to eq true
      ...
      @res3 = TonSdk::Crypto.mnemonic_verify(@c_ctx.context, pr3)
      expect(@res3.success?).to eq true
    ...
    it "#chacha20" do
      ...
      @res = TonSdk::Crypto.chacha20(@c_ctx.context, pr1)
    ...
    it "#signing_box" do
      ...
      @res1 = TonSdk::Crypto.get_signing_box(@c_ctx.context, kp)
      ...
      @res2 = TonSdk::Crypto.signing_box_get_public_key(@c_ctx.context, reg_sb)
      ...
      @res3 = TonSdk::Crypto.signing_box_sign(@c_ctx.context, a3)
      ...
      # 4
      @res4 = TonSdk::Crypto.register_signing_box(@c_ctx.context, nil)
      timeout_at = get_timeout_for_async_operation()
    ...
    it "#nacl" do
      ...
      @res = TonSdk::Crypto::nacl_sign_keypair_from_secret_key(@c_ctx.context, pr1)
      expect(@res.success?).to eq true
      ...
      @res2 = TonSdk::Crypto::nacl_sign(@c_ctx.context, pr2)
      expect(@res2.success?).to eq true
      ...
      @res3 = TonSdk::Crypto::nacl_sign_open(@c_ctx.context, pr3)
      expect(@res3.success?).to eq true
